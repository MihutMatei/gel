...
class AuthRepository(
    ...
    fun mapErrorMessage(throwable: Throwable): String {
        ...
        // Ktor JS often wraps network/CORS errors; message can be empty.
        return buildString {
            append(msg ?: "Request failed")
            append(" (baseUrl=")
            append(base)
            append(")")
            val causeMsg = throwable.cause?.message?.takeIf { it.isNotBlank() }
            if (causeMsg != null && causeMsg != msg) {
                append("; cause=")
                append(causeMsg)
            }
        }
