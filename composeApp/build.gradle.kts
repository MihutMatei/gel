...
val apiBaseUrl = (dotEnv["API_BASE_URL"] ?: System.getenv("API_BASE_URL") ?: "").trim()
...
// --- Web runtime config injection (JS) ---
val generateWebRuntimeConfig by tasks.registering {
    val outDir = layout.buildDirectory.dir("generated/web-config")
    outputs.dir(outDir)

    doLast {
        val file = outDir.get().file("runtime-config.js").asFile
        file.parentFile.mkdirs()

        val escapedKey = mapTilerApiKey.replace("\\", "\\\\").replace("\"", "\\\"")
        val escapedApiBaseUrl = apiBaseUrl.replace("\\", "\\\\").replace("\"", "\\\"")

        file.writeText(
            """
            // Generated by Gradle. Do not commit.
            (function(){
              window.__MAPTILER_API_KEY__ = "$escapedKey";
              window.__API_BASE_URL__ = "$escapedApiBaseUrl";
            })();
            """.trimIndent()
        )
    }
...
// Make sure the generated file ends up in processedResources for js/web.
// We attach to the JS processResources task which is used by the browser distribution.
tasks.matching { it.name == "jsProcessResources" }.configureEach {
    dependsOn(generateWebRuntimeConfig)
    doLast {
        val generated = layout.buildDirectory.dir("generated/web-config").get().asFile
        val targetDir = layout.buildDirectory.dir("processedResources/js/main").get().asFile
        generated.copyRecursively(targetDir, overwrite = true)
    }
}
